...
// ---------- THEME (dark) ----------
const INPUT_CLS =
  'w-full bg-slate-900/90 border border-slate-700 rounded-2xl px-4 py-2 text-slate-100 placeholder-slate-500 focus:outline-none focus:border-cyan-400';
const BTN_PRIMARY =
  'px-5 py-2.5 rounded-2xl bg-cyan-500 text-slate-950 font-semibold hover:bg-cyan-400 transition';
const BTN_CANCEL =
  'px-5 py-2.5 rounded-2xl border border-slate-700 text-slate-300 hover:bg-slate-800 transition';
const CARD_CLS = 'bg-slate-900/95 border border-slate-800 rounded-3xl shadow-2xl';
const DIVIDE_CLS = 'divide-y divide-slate-800';
const MUTED = 'text-slate-400';
const LINK_CLS = 'text-cyan-300 hover:text-cyan-200 hover:underline';
const DANGER_LINK = 'text-rose-400 hover:text-rose-300 hover:underline';

const statusLabel = { todo: 'To Do', in_progress: 'In Progress', done: 'Done' };
const statusColor = {
  todo: 'bg-slate-700 text-slate-200 border border-slate-600',
  in_progress: 'bg-cyan-600/15 text-cyan-200 border border-cyan-500/30',
...
function bindAuthScreen() {
  ...
  function showTab(which) {
    showAuthError('');
    if (which === 'login') {
      tabLogin.classList.add('bg-slate-950', 'text-slate-100');
      tabLogin.classList.remove('bg-slate-900', 'text-slate-400');
      tabSignup.classList.remove('bg-slate-950', 'text-slate-100');
      tabSignup.classList.add('bg-slate-900', 'text-slate-400');
      loginForm.classList.remove('hidden');
      signupForm.classList.add('hidden');
    } else {
      tabSignup.classList.add('bg-slate-950', 'text-slate-100');
      tabSignup.classList.remove('bg-slate-900', 'text-slate-400');
      tabLogin.classList.remove('bg-slate-950', 'text-slate-100');
      tabLogin.classList.add('bg-slate-900', 'text-slate-400');
  ...
  try {
    ...
    main.innerHTML = `
      <div class="flex flex-col gap-4 md:flex-row md:items-end md:justify-between mb-4">
        <div>
          <h2 class="text-3xl font-bold mb-1 text-slate-100">FlowTeam Dashboard</h2>
          <p class="text-sm ${MUTED} max-w-2xl">A modern workspace that keeps your team aligned, productive, and on schedule.</p>
        </div>
        <button id="dashboard-action-btn" class="${BTN_PRIMARY}">Create project</button>
      </div>

      <div class="grid grid-cols-1 md:grid-cols-3 gap-3 mb-6">
        ${statCard('Projects', projectsCount, 'bg-cyan-500/10 text-cyan-200 border border-cyan-500/20')}
        ${statCard('Total Tasks', counts.total, 'bg-slate-800 text-slate-200 border border-slate-700')}
        ${statCard('Open Tasks', counts.todo + counts.in_progress, 'bg-slate-700/60 text-slate-200 border border-slate-600')}
        ${statCard('To Do', counts.todo, 'bg-slate-700/60 text-slate-200 border border-slate-600')}
        ${statCard('In Progress', counts.in_progress, 'bg-cyan-500/10 text-cyan-200 border border-cyan-500/30')}
    ...
    `;
    $('#dashboard-action-btn').onclick = openNewProjectModal;
  } catch (err) {
    ...
    main.innerHTML = `
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-4">
        <div>
          ...
          <p class="text-sm ${MUTED}">Browse your active project workspace and team assignments.</p>
        </div>
        <button id="new-project-btn" class="${BTN_PRIMARY}">+ Create project</button>
      </div>
      ${projects.length === 0
        ...
        : `<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">${projects
            .map(
              (p) => `
          <div class="${CARD_CLS} p-5 hover:border-cyan-400/50 hover:shadow-cyan-400/10 cursor-pointer transition project-card" data-id="${p.id}">
    ...
    main.innerHTML = `
      ...
      <div class="flex flex-wrap items-start justify-between gap-3 mb-4">
        ...
        <div class="flex flex-wrap gap-2">
          ...
          ${isAdmin ? `<button id="new-task-btn" class="bg-cyan-500 text-slate-950 px-3 py-2 rounded-lg text-sm font-semibold hover:bg-cyan-400 transition">+ Task</button>` : ''}
          ${isAdmin ? `<button id="delete-project-btn" class="bg-rose-600/90 text-white px-3 py-2 rounded-lg text-sm font-semibold hover:bg-rose-500 transition">Delete</button>` : ''}
        </div>
      </div>
  ...
  return `
    <div class="border border-slate-700 bg-slate-800/40 rounded-lg p-3 flex flex-wrap items-center justify-between gap-3 hover:border-slate-600 transition">
      ...
      <div class="flex items-center gap-2">
        ${canChangeStatus
          ? `<select data-id="${t.id}" class="task-status-select bg-slate-800 border border-slate-700 text-slate-100 rounded px-2 py-1 text-sm focus:outline-none focus:border-cyan-400">${statusOptions}</select>`
